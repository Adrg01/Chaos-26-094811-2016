...
if strcmp(action,'initialize')
  ...
  system.xvar = 'x';
  system.yvar = 'y';
  system.xder = '0.5*(k*x +f)*(1+y^2-x^2) +k*x*y*y + o*y -d*x';
  system.yder = '0.5*(3*k*x*x*y + 2*f*x*y -k*y*y*y) - (o*x + d*y)';
  system.pname = {};
  system.pval = {};
  ...
  system(11).name = 'nld project';
  system(11).xvar = 'x';
  system(11).yvar = 'y';
  system(11).xder = '0.5*(a*x +b)*(1+y*y-x*x) - a*x*y*y +c*y - x';
  system(11).yder = '-0.5*(a*x*x*y + 2*b*x*y + a*y*y*y -a*y) -c*x -y';
  system(11).pname = {'a', 'b', 'c', '', '', ''};
  system(11).pval = {'10', '3.5', '1.4', ''};
  system(11).fieldtype = 'arrows';
  system(11).npts = npts;
  system(11).wind = [-1 1 -1 1];
